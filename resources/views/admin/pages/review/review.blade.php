<x-admin.index :user="$user" :isAdmin="$isAdmin">
	<div class="content-wrapper food_btn_add">
		<a href="{{ route('testimonial.create') }}" class="btn btn-primary mx-2">{{ __('admin.add_testimonial') }}
		...
		<div class="col-lg-12 grid-margin stretch-card">
			<div class="card">
				<div class="card-body food_body">
					<h4 class="card-title">{{ __('admin.testimonial_table') }}</h4>					
					<p class="card-description">
						{{ __('admin.testimonial_table_desc') }}
					</p>
					...
					<table class="table table-hover overflow-auto block">
						<thead>
							<tr class="bg-slate-800">
								@foreach(["Image", "Name", "Bio", "Rating", "Description","Created_at", "edit_header", "delete_header"] as $heading)
						...
						<tbody>
							...
							<tr>
								...
								<td>
										<a style="padding: 8px 12px" href="{{ route('testimonial.edit', $data->id) }}" class="badge badge-primary cursor-pointer">
								...
								<td>
									@if ($isAdmin === true)
								  	<form id="delete-form-{{ $data->id }}" action="{{ route('testimonial.destroy', $data->id) }}" method="POST" style="display: none;">
									@csrf
									@method('DELETE')
								</form>
