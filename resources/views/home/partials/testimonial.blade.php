    ...
    <section id="testimonial" class="bg-[url(assets/images/testimonial-bg.jpg')] bg-center bg-cover bg-no-repeat mt-28 px-10 py-24 relative w-full z-1">
      ...
      <div class="m-4 max-w-md mt-5 p-4 text-center text-white mx-auto">
          ...
          @if (App::getLocale() == 'ar')
              {{ __('home.says') }} <span class="text-amber-400 leading-snug">{{ __('home.clients') }}</span>...
          @else
              {{ __('home.clients') }} <span class="text-amber-400 leading-snug">{{ __('home.says') }}</span> ...
          @endif
      </h3>
        <p class="pt-2 text-[14px] font-sans-lato text-slate-100 leading-relaxed">{{ __('home.clients_desc') }}</p>
      </div>
