    ...
    <section class="py-32 mx-auto max-w-[960px]">
      <div class="m-4 max-w-md mt-5 mb-11 p-4 text-center mx-auto">
       <h3 class="font-bold font-cursive-merie text-4xl leading-normal capitalize">
          @if (App::getLocale() == 'ar')
              {{ __('home.dishes') }} <span class="text-amber-400 leading-snug">{{ __('home.fav') }}</span>
          @else
              {{ __('home.our') }} <span class="text-amber-400 leading-snug">{{ __('home.fav') }}</span> {{ __('home.dishes') }}
          @endif
      </h3>

        <p class="pt-2 text-[14px] font-sans-lato text-slate-600 leading-relaxed">{{ __('home.dishes_desc') }}</p>
      </div>
